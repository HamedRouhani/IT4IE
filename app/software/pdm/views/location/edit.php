<?php
/**
 * PdM Analyzer - فرم ویرایش مکان
 * مسیر: app/software/pdm/views/location/edit.php
 */
?>
...
<div class="software-content pdm-fade-in">
    ...
    <div class="pdm-flex-between pdm-mb-4">
        <div>
            <h2 style="color: var(--pdm-primary-dark); margin: 0;">
                <i class="fas fa-edit"></i> ویرایش مکان
            </h2>
            <p class="pdm-text-muted pdm-mt-2" style="margin: 0;">
                ویرایش: <strong><?= pdm_e($location['name']) ?></strong>
            </p>
        </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="<?= pdm_url('location', 'update', ['id' => $location['id']]) ?>">
                <?= $this->csrfField() ?>
                    ...
                    <select id="parent_id" name="parent_id" class="pdm-form-control">
                        ...
                        <?php foreach ($parentOptions as $opt): ?>
                            <option value="<?= (int) $opt['id'] ?>"
                                <?= (int) ($location['parent_id'] ?? 0) === (int) $opt['id'] ? 'selected' : '' ?>>
                                <?= str_repeat('— ', (int) ($opt['level'] ?? 0)) . pdm_e($opt['name']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <small class="pdm-text-muted">توجه: نمی‌توانید مکان را والد خودش قرار دهید.</small>
                </div>
                ...
                <div class="pdm-form-group">
                    ...
                    <textarea id="description"
                              ...
                              class="pdm-form-control"
                              rows="3"
                              maxlength="1000"><?= pdm_e($location['description'] ?? '') ?></textarea>
                </div>
