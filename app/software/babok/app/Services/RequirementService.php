...
/**
 * سرویس استخراج و تحلیل یکپارچه نیازمندی
 * 
 * قابلیت‌ها:
 * - استخراج نیازمندی‌های عملکردی و غیرعملکردی از متن فارسی
 * - شناسایی حوزه دانشی مرتبط (KA1 تا KA6)
 * - شناسایی حوزه کاری (مالی، تولید، خدمات و...)
 * - امتیازدهی و پیشنهاد تکنیک‌های BABOK
 * - تولید پیشنهاد سیستم بر اساس حوزه کاری
 */
class RequirementService
{
    ...
    private $keywordWeights = [
        ...
        'مشکل' => 2, 'چالش' => 2, 'نیاز' => 3,
        'فرصت' => 2, 'تهدید' => 2, 'نقاط قوت' => 2, 'نقاط ضعف' => 2
    ];

    // ============================================================
    // الگوهای تشخیص جملات نیازمندی
    // ============================================================
    ...
    // ============================================================
    // نگاشت عبارات به نیازمندی‌های خاص
    // ============================================================
    private $phraseMapping = [
        'مسائل مالی' => [
        ...
        'حقوق' => [
            ...
            'type' => 'functional',
            'description' => 'سیستم باید امکان محاسبه خودکار حقوق و دستمزد، مدیریت کسورات، صدور فیش حقوقی و گزارش‌دهی قانونی را فراهم کند.',
            'babok_reference' => 'مدیریت حقوق و دستمزد'
        ],
        'فروش' => [
            'title' => 'سیستم مدیریت فروش و مشتریان',
            'type' => 'functional',
            'description' => 'سیستم باید امکان مدیریت چرخه فروش، پیگیری فرصت‌ها، مدیریت ارتباط با مشتریان، تحلیل فروش و گزارش‌دهی را فراهم کند.',
            'babok_reference' => 'مدیریت فروش و مشتریان'
        ],
        'بازاریابی' => [
            'title' => 'سیستم مدیریت بازاریابی',
            'type' => 'functional',
            'description' => 'سیستم باید امکان برنامه‌ریزی کمپین‌های بازاریابی، تحلیل بازار، مدیریت سرنخ‌ها و اندازه‌گیری اثربخشی تبلیغات را فراهم کند.',
        ...
        'امنیت' => [
            'title' => 'سیستم امنیت اطلاعات',
            'type' => 'non_functional',
            'description' => 'سیستم باید دارای مکانیزم‌های امنیتی قوی شامل رمزنگاری داده‌ها، احراز هویت چندعاملی، کنترل دسترسی و ثبت وقایع امنیتی باشد.',
            'babok_reference' => 'امنیت اطلاعات'
        ],
        'عملکرد' => [
            'title' => 'نیازمندی عملکرد سیستم',
            'type' => 'non_functional',
            'description' => 'سیستم باید پاسخگوی درخواست‌ها در زمان مناسب باشد و عملکرد قابل قبولی تحت بار کاری بالا داشته باشد.',
            'babok_reference' => 'عملکرد سیستم'
        ]
    ];
    ...
    private $domainSuggestions = [
        ...
        'فناوری اطلاعات' => [
            'title' => 'سیستم مدیریت خدمات فناوری اطلاعات (ITSM)',
            'description' => 'سیستمی برای مدیریت زیرساخت، خدمات و پشتیبانی فناوری اطلاعات'
        ]
    ];
     ...
     * @return array نتایج تحلیل شامل نیازمندی‌ها، تکنیک‌ها، حوزه و آمار
     */
    ...
    private function detectDomain(string $text): string
    {
        $domains = [
            ...
            'تحقیق و توسعه' => ['تحقیق', 'توسعه', 'نوآوری', 'آزمایشگاه']
        ];
    ...
    private function getDomainSuggestion(string $domain, string $text): ?array
    {
        ...
        foreach ($this->domainSuggestions as $key => $suggestion) {
            $score = 0;
            $words = mb_str_split($key, 1);
            
            // بررسی تطابق کامل کلید
            ...
            foreach (explode(' ', $key) as $word) {
                if (mb_strlen($word) > 1 && mb_stripos($text, $word) !== false) {
                    $score += 3;
                }
            }
    ...
    private function detectRequirementType(string $sentence): string
    {
        ...
        $functionalKeywords = [
            'کاربر', 'پرداخت', 'گزارش', 'مدیریت', 'جستجو', 'ذخیره', 'ویرایش', 'حذف', 'ورود', 'ثبت', 'روابط', 'ارتباط', 'همکاری', 'تیم',
            'مالی', 'حسابداری', 'فاکتور', 'حسابرسی', 'تولید', 'انبار', 'خدمات'
        ];
    ...
    {
        // حذف کاراکترهای اضافی (فقط حروف، اعداد و فاصله باقی بمانند)
    ...
    private function detectKnowledgeArea(string $text): ?array
    {
        ...
        return ($topArea && $scores[$topArea] > 0) ? $this->knowledgeAreas[$topArea] : null;
    }
    ...
    private function getDetailedReason(array $technique, array $keywords, ?array $knowledgeArea): string
    {
        ...
        $specificReasons = [
            ...
            'Benchmarking and Market Analysis' => 'برای مقایسه با بهترین شیوه‌های مالی در صنعت'
        ];
