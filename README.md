...
### 6. 🏭 **PdM Analyzer** (نگهداری و تعمیرات پیشگویانه) — *جدید در نسخه 9.0*
...
- **انواع نگهداری**: CM (اصلاحی)، PM (پیشگیرانه)، PdM (پیشگویانه)، CBM (مبتنی بر وضعیت)، EM (اضطراری)، RCM (مبتنی بر قابلیت اطمینان)
- **دستورکارها (Work Orders)**: قلب سیستم با چرخه حیات کامل (باز → در حال انجام → تکمیل شده → لغو شده)
...
- **گزارش‌های حرفه‌ای**: ۵ گزارش کامل با قابلیت چاپ (دستورکارها، خرابی‌ها، KPI، موجودی، برنامه‌های نت)

#### 🏗️ معماری فنی
- **Multi-Tenant**: هر کاربر یک شرکت (One-to-One)
- **حداقل ۲۱ جدول** با پیشوند `pm_`
- **Namespace اختصاصی**: `App\Software\Pdm\`
- **تقویم شمسی**: تبدیل خودکار تاریخ با توابع داخلی (مستقل از وابستگی‌های خارجی)
...
- **۳ فایل CSS/JS** اختصاصی (شامل Datepicker شمسی سبک)
- **بیش از ۵۰۰۰ خط کد** PHP/CSS/JS
